  ...
  static Future<void> _seedUP16Shop(FirebaseFirestore firestore) async {
    ...
    if (!up16Doc.exists) {
      await up16Ref.set({
        ...
        'bannerUrl': 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=500',
        'contactNumber': '8745007244',
        'orderNumber': '8745007244',
        'openTime': '08:00 AM',
        'closeTime': '11:30 PM',
        'isClosedOverride': false,
        'isActive': true,
      ...
      if (!data.containsKey('minimumOrderAmount')) patch['minimumOrderAmount'] = 0;
      if (!data.containsKey('name') || (data['name'] as String).isEmpty) {
        patch['name'] = 'UP 16 Junction Fast Food';
      }
      if (patch.isNotEmpty) {
    ...
    for (final cat in up16Categories) {
      await up16Ref.collection('categories').doc(cat['id'] as String).set({
        'name': cat['name'],
        'imageUrl': cat['imageUrl'],
        'sortOrder': cat['sortOrder'],
    ...
    // Complete UP 16 Cleaned Menu Items (60 unique entries with Half/Full variants where applicable)
    final up16Items = [
      // ─── 1. Rolls ─────────────────────────────────────────────────────────
    ...
    for (int i = 0; i < up16Items.length; i++) {
      final item = up16Items[i];
      final docId = item['id'] as String;
      await up16Ref.collection('menuItems').doc(docId).set({
        'name': item['name'],
        'details': item['details'],
        'price': item['price'],
        'imageUrl': '',
